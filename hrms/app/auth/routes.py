...
def change_password(
    ...
    service.change_password(db, current_user, data.old_password, data.new_password)
...
def forgot_password(data: schemas.ForgotPasswordRequest, db: Session = Depends(get_db)):
    ...
    return APIResponse(code=200, message="OTP sent successfully", data=None)
...
def verify_otp(data: schemas.VerifyOTPRequest, db: Session = Depends(get_db)):
    ...
    return APIResponse(code=200, message="OTP verified successfully", data=None)
...
def reset_password(data: schemas.ResetPasswordRequest, db: Session = Depends(get_db)):
    ...
    return APIResponse(code=200, message="Password reset successfully", data=None)
