...
def update_hiring_request(
    ...
    if update_fields.get("status") == "Approved":
        ...
        existing_posting = db.query(JobPosting).filter(
            JobPosting.hiring_request_id == hiring.id
        ).first()
    for key, value in update_fields.items():
        setattr(hiring, key, value)

    # =================================================
    # Manager Approval
    # =================================================

    if update_fields.get("manager_approved") == True:

        if hiring.manager_id != employee.id:
            raise HTTPException(
                403,
                "Only the assigned manager can approve this hiring request"
            )

        hiring.manager_approved = True
        hiring.manager_approved_at = datetime.utcnow()
        hiring.status = "Approved"

        # create job posting automatically
        existing_posting = db.query(JobPosting).filter(
            JobPosting.hiring_request_id == hiring.id,
...
def get_all_job_postings(
    ...
    query = db.query(JobPosting).join(HiringRequest)
